...
namespace HR_PAYROLL_PROCESSING_SYSTEM.Master
{
    public partial class ErrorCodeMasterListing : System.Web.UI.Page
    {
        ...
        protected void grid1_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                ErrorCodeMasterEntity objErrCodeMaster = new ErrorCodeMasterEntity();
                GridViewRow row = grid1.Rows[e.RowIndex];
                string errCode = (row.FindControl("lblErrCode") as Label).Text;

                int s = objErrorCodeManager.DeleteOption(errCode);
                if (s > 0)
                {
        ...
        protected void grid1_RowEditing(object sender, GridViewEditEventArgs e)
        {
            try
            {
                string code = grid1.DataKeys[e.NewEditIndex].Values["ERR_CODE"].ToString();
                Response.Redirect("ErrorCodeMaster.aspx?pCode=" + code);
            }
            catch (Exception)
            {
                ErrorCodeMasterManager objErrorCodeMasterManager = new ErrorCodeMasterManager();
        ...
        protected void grid1_Sorting(object sender, GridViewSortEventArgs e)
        {
            try
            {
                ...
                ErrorCodeMasterManager objErrCodeMasterMgr = new ErrorCodeMasterManager();
                DataTable dt = objErrCodeMasterMgr.LoadGridDetails();
